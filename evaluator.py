...
def get_all_maxima_times(spectrum, times, threshold, printing=False):
...
def get_note_predictions(pitches, spectrum, times, threshold):
    all_maxima_times = get_all_maxima_times(spectrum, times, threshold)
    note_predictions = np.empty(len(pitches), dtype=object)
    for i in range(len(pitches)):
        note_predictions[i] = (pitches[i], all_maxima_times[i])
    return note_predictions
...
def get_pitch_probabilities(wav_name, model, number_of_pitches=89, plotting=False, printing=False):
    ...
    spectrum, _, times, _ = get_spectrogram(f'scratch-wav-files/{wav_name}.wav')
...
def main():
    ...
    # print(get_all_maxima_times(spectrum, t, 0.7))
    ...
    get_pitch_probabilities('A4', model, plotting=True)
